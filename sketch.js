...
function setup() {
  ...
  gameOver = createSprite(width - 100,height - 400);
  gameOver.addImage(gameOverImg);
  
  restart = createSprite(width - 100, height - 400);
...
function draw() {
  ...
  if (gameState===PLAY){
    ...
    if((touches.length > 0 || keyDown("space")) && trex.y >= 380) {
      trex.velocityY = -12;
      jumpSound.play();
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    var cloud = createSprite(width - 50,440,40,10);
    cloud.y = Math.round(random(290,320));
...
function spawnObstacles() {
  if(frameCount % 60 === 0) {
    var obstacle = createSprite(width - 50,380,10,40);
    //obstacle.debug = true;
